...
class msPrimeSimulate():
    ...
    def main(self):
        ...
        coalescent_TwoEpC = '{0}{1}TwoEpochContraction_{2}_coaldist_{3}.txt'
        output_TwoEpE = '{0}{1}TwoEpochExpansion_{2}_{3}.vcf'.format(
            args['outprefix'], underscore, sample_size, replicate)
        coalescent_TwoEpE = '{0}{1}TwoEpochExpansion_{2}_coaldist_{3}.txt'
        output_ThreeEpC = '{0}{1}ThreeEpochContraction_{2}_{3}.vcf'.format(
            args['outprefix'], underscore, sample_size, replicate)
        coalescent_ThreeEpC = '{0}{1}ThreeEpochContraction_{2}_coaldist_{3}.txt'
        output_ThreeEpE = '{0}{1}ThreeEpochExpansion_{2}_{3}.vcf'.format(
            args['outprefix'], underscore, sample_size, replicate)
        coalescent_ThreeEpE = '{0}{1}ThreeEpochExpansion_{2}_coaldist_{3}.txt'
        output_ThreeEpB = '{0}{1}ThreeEpochBottleneck_{2}_{3}.vcf'.format(
            args['outprefix'], underscore, sample_size, replicate)
        coalescent_ThreeEpB = '{0}{1}ThreeEpochBottleneck_{2}_coaldist_{3}.txt'
        to_remove = [logfile]
        for f in to_remove:
            if os.path.isfile(f):
                os.remove(f)
        ...
        with open(output_TwoEpC, "w+") as f0:
            ...
            tree_0 = ts0.first()
            logger.info('First tree in TwoEpC: {0}'.format(tree_0.as_string()))
        ...
        with open(output_TwoEpE, "w+") as f1:
            ...
            tree_1 = ts1.first()
            logger.info('First tree in TwoEpE: {0}'.format(tree_1.as_string()))
        ...
        with open(output_ThreeEpC, "w+") as f2:
            ...
            tree_2 = ts2.first()
            logger.info('First tree in ThreeEpC: {0}'.format(tree_2.as_string()))
        ...
        with open(output_ThreeEpE, "w+") as f3:
            ...
            tree_3 = ts3.first()
            logger.info('First tree in ThreeEpE: {0}'.format(tree_3.as_string()))
        ...
        with open(output_ThreeEpB, "w+") as f4:
            ...
            tree_4 = ts4.first()
            logger.info('First tree in ThreeEpB: {0}'.format(tree_4.as_string()))
