...
# figure_s5A_dataframe$sample_size = sample_size

figure_s5A_dataframe = melt(data.frame(
  growth_coal_proportion,
  bottleneck_coal_proportion,
  ancestral_coal_proportion
))

figure_s5A_dataframe$msprime_time = msprime_time
figure_s5A_dataframe$msprime_shape = msprime_nu_shape
figure_s5A_dataframe$sample_size = sample_size

plot_3A = ggplot(data=figure_s5A_dataframe, aes(x=sample_size, y=value, color=variable, shape=msprime_shape)) +
  geom_point(size=1.5) +
  theme_bw() +
  # xlab('Sample size') +
...
figure_s5B = ggplot(branch_proportion_df,
                     aes(x = sample_size,
                         y = value,
                         color = epoch,
                       shape=shape)) +
...
# 700x600
figure_3 = plot_3A + figure_s5B + plot_layout(nrow=2)
