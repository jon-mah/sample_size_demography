...
class msPrimeSimulate():
    ...
    def main(self):
        ...
        output_contraction = '{0}{1}contraction_{2}.vcf'.format(args['outprefix'], underscore, replicate)
        output_expansion = '{0}{1}expansion_{2}.vcf'.format(args['outprefix'], underscore, replicate)
        output_bottleneck = '{0}{1}bottleneck_{2}.vcf'.format(args['outprefix'], underscore, replicate)
        to_remove = [logfile]
        for f in to_remove:
            if os.path.isfile(f):
                os.remove(f)
